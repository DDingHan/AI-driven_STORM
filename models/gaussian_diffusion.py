...
class GaussianDiffusion:
    ...
    def training_losses(
        ...
        #x_start = GT / y = LR / residual = y - x_start = LR - HR
        z_y = self.encode_first_stage(y, first_stage_model, up_sample=True)
        ...
        if self.loss_type == LossType.MSE or self.loss_type == LossType.WEIGHTED_MSE:
            ...
            #residual loss 
            residual_pred = z_y - model_output
            residual_gt = z_y - z_start
            terms["residual_mse"] = mean_flat((residual_gt - residual_pred) ** 2)
            ...
            terms["mse"] = mean_flat((target - model_output) ** 2) + 0.5 * terms["residual_mse"]
